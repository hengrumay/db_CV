...
# MAGIC
# MAGIC Nice to add: 
# MAGIC - MLflow Model logging | [YY's testing](https://adb-984752964297111.11.azuredatabricks.net/editor/notebooks/455928964387434?o=984752964297111#command/3340647220341614) + [env_vars for Databricks host+token](https://databricks.slack.com/archives/C07CWR6L0BB/p1732053989469349?thread_ts=1732040412.080019&cid=C07CWR6L0BB)
# MAGIC
# MAGIC ---     
# MAGIC
# MAGIC
# MAGIC [INTERNAL](https://adb-984752964297111.11.azuredatabricks.net) YOLO Devs & Refs:
# MAGIC - [mlflow_CustomPyfunc_wrapper_for_YOLO](https://adb-984752964297111.11.azuredatabricks.net/editor/notebooks/455928964387434?o=984752964297111#command/3340647220341614) | [WhimsicalFish_run](https://adb-984752964297111.11.azuredatabricks.net/ml/experiments/3340647220336346/runs/109c4360e1e94e69a1295ca6298ac62e?o=984752964297111)
# MAGIC - [Computer Vision: YOLO Training Best Practice on Databricks_YY](https://adb-984752964297111.11.azuredatabricks.net/editor/notebooks/2705005333728677?o=984752964297111#command/2705005333728678)
# MAGIC
# MAGIC - [GY's examples](https://adb-984752964297111.11.azuredatabricks.net/editor/notebooks/3869932902533013?o=984752964297111#command/455928964337642)
# MAGIC
# MAGIC <!-- EXTERNAL Refs: 
# MAGIC - [YOLOv8 instance segmentation transfer learning example](https://towardsdatascience.com/trian-yolov8-instance-segmentation-on-your-data-6ffa04b2debd) 
# MAGIC - [Databricks MLflow integration snippets](https://benhay.es/posts/object_detection_yolov8/) 
...
# DBTITLE 1,Example Code Snippet
# use best weights to define and log/register model 
...
# e.g. Define a custom model class
############################################################################
...
class YOLOC(mlflow.pyfunc.PythonModel):
  ...
  def load_context(self, context):
    from ultralytics import YOLO
    self.model = YOLO(context.artifacts['best_point']) ## update this to point to your point file
    # self.model = YOLO("dbfs:/databricks/mlflow-tracking/3420142641041077/59cb657f866a4cae9307de91de9cb1d9/artifacts/weights/best.pt")
    
  def predict(self, context, model_input):
      results = self.model(model_input)
      return results.pandas().xyxy[0]  # Return predictions as a pandas DataFrame || update this to reflect your model output



############################################################################
## Log the YOLOC class model ##
############################################################################

# Log the model
with mlflow.start_run() as run:
    mlflow.pyfunc.log_model("yolo_model", python_model=YOLOC(),
                            #signature, 
                            #example_input,                                
                            )
